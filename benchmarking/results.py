...
def persistMetricsAsCSV(workloadType: WorkloadTypes, allMetrics: dict, workloadName: str, indexType: IndexTypes):
    ...
    fields = ["workload-name", "indexType", "dataset-name", "dimensions", "vectors-count", "queries-count", "indexing-params", "index-creation-time", "gpu-to-cpu-index-conversion-time", "write-to-file-time", "write-index-time", "total-build-time", "peak-gpu-memory-usage", "leftover-gpu-memory", "peak-cpu-memory-usage", "leftover-cpu-memory", "search-parameter", "search-time", "unit", "search-throughput", "recall@100", "recall@1"]
    rows = []
    if workloadType == WorkloadTypes.INDEX:
        ...
        for searchMetric in allMetrics[workloadName]["searchMetrics"]:
            ...
            if allMetrics[workloadName].get("indexingMetrics") is not None:
                ...
                row["peak-cpu-memory-usage"] = formatTimingMetricsValue(indexingMetrics[indexingParamItr]["memory_metrics"]["peak_cpu_mem"])
                row["leftover-cpu-memory"] = formatTimingMetricsValue(indexingMetrics[indexingParamItr]["memory_metrics"]["leftover_cpu_mem"])
